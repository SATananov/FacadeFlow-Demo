...
FacadeFlow Demo Phase 01 + Phase 02A/02B/02C/02D е локален визуален прототип. Единственият файлов export е четим JSON с `simulationOnly: true` и разширение `.simulation.json`. Прегледът на изделието може да се отпечата само чрез стандартния диалог на браузъра.
