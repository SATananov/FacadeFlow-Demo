...
## Експертно потвърдена последователност

- Видимият процес е: общи размери → външна каса → вертикални/хоризонтални делители → крила в избрани валидни полета → посоки само на съществуващи крила → човешка проверка → концептуален 3D преглед.
- Невалидни общи размери блокират създаването на каса; липсваща каса блокира делителите и крилата.
- Фиксираното остъкляване не е крило. Крило се създава с отделно действие и изборът на посока никога не създава крило автоматично.
- Посока не може да се зададе без създадено крило и валиден SASH профил. Премахването на крилото премахва и посоката.
- Текущата стъпка и непълните предходни изисквания са видими. 3D е достъпен едва след `VERIFIED` човешка проверка.
- Recursive split tree и stable component IDs не се променят от въвеждането на стъпковия процес.
- Всички REF шаблони са shortcuts към същия ordered structured model. Проверените импортни записи се преобразуват в него след човешка проверка.
- Моделът не добавя производствени формули, профилни отнемания, machining operations или machine output.
