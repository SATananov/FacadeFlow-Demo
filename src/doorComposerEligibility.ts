...
export function getDoorComposerEligibility(configuration: StructuredProfileConfiguration, acknowledged: boolean, profiles: CatalogueProfile[]): DoorComposerEligibility {
  ...
  if (!compatibleProfiles(profiles, configuration.profileSystem, 'FRAME', configuration.productCategory).some(p => p.id === configuration.frameProfileId)) blockers.push('Изберете съвместим DEMO профил за каса.')
  if (!compatibleProfiles(profiles, configuration.profileSystem, 'SASH', configuration.productCategory).some(p => p.id === configuration.sashProfileId)) blockers.push('Изберете съвместим DEMO профил за крило.')
