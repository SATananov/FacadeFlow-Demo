...
export function validateCatalogueProfile(profile: CatalogueProfile, all: CatalogueProfile[]): CatalogueValidation {
  ...
  if (profile.sourceEvidenceId && profile.humanRoleReviewStatus === 'HUMAN_CONFIRMED' && !profile.humanRoleConfirmedBy?.trim()) errors.push('За роля, потвърдена от човек, е задължително да е посочено кой човек / технолог я е потвърдил.')
