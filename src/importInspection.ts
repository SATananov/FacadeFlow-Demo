...
export async function inspectImportFile(file: File, selectedRoute: ImportRoute, maximumBytes: number): Promise<{ bytes: ArrayBuffer; inspection: InspectedImportSource }> {
  ...
  if (detected.format === 'UNKNOWN') warnings.push('Форматът не може да бъде потвърден безопасно по съдържанието.')
    ...
    const strictMime: Partial<Record<typeof detected.format, string[]>> = { PNG: ['image/png'], JPEG: ['image/jpeg'], PDF: ['application/pdf'], SKYGLAZING_XML: ['application/xml', 'text/xml'], LTE: ['text/plain'], CSV: ['text/csv'], XLSX: ['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'], FACADEFLOW_SIMULATION_JSON: ['application/json'] }
  ...
  if (!warnings.length) supportStatus = selectedRoute === 'IMAGE' || selectedRoute === 'PDF' ? 'SUPPORTED' : selectedRoute === 'CAD' || selectedRoute === 'SKYGLAZING' ? 'SUPPORTED_FOR_VIEW_ONLY' : 'FUTURE_SUPPORT'
