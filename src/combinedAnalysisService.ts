...
import { ocrProgressStatusBg } from './importUiLabels'
...
export async function runCombinedAnalysis(selection: OcrSelection, sourceSha256: string, onProgress: (message: string) => void, register: (control: OcrRunControl) => void): Promise<CombinedAnalysisJob> {
  ...
  onProgress('Извличане на демонстрационни геометрични признаци…')
  ...
  const ocr = await runLocalOcr(selection.imageDataUrl, (progress, status) => onProgress(`${ocrProgressStatusBg(status)} ${Math.round(progress * 100)}%`), register)
