...
export const composerFieldDisplayName = (fields: VisualComposition['fields'], id: string) => { const ordered = [...fields].sort((a, b) => a.rect.y - b.rect.y || a.rect.x - b.rect.x), index = ordered.findIndex((field) => field.id === id); if (index < 0) return 'Неизвестно поле'; if (ordered.length === 1) return 'Поле'; if (ordered.length === 2 && ordered[0]?.rect.y === ordered[1]?.rect.y) return index === 0 ? 'Ляво поле' : 'Дясно поле'; return `Поле ${index + 1} от ${ordered.length}` }
