...
import type { ComposerComponent, ComposerItemType, ComposerOpeningDirection, VisualComposition } from './visualComposerTypes'
...
export const createEmptyComposition = (): VisualComposition => ({ templateId: null, fields: [], components: [], selectedFieldId: null, selectedComponentId: null, status: 'EMPTY', message: 'Изберете демонстрационен шаблон.', ...safety })
...
export function applyComposerItem(state: VisualComposition, item: ComposerItemType, idFactory: () => string, positionRatio = .5): VisualComposition {
  ...
  if (directions[item]) { fields[index] = { ...field, openingDirection: directions[item]! }; return changed(state, { fields, message: 'Посоката на отваряне е приложена.' }) }
  if (item === 'HANDLE') {
    if (field.attachedHandleId) return reject('Полето вече има демонстрационна дръжка.')
    const id = idFactory(); fields[index] = { ...field, attachedHandleId: id }; components.push(component(id, 'HANDLE', field.id, 'HANDLE', 'среда')); return changed(state, { fields, components, selectedComponentId: id, message: 'Добавена е демонстрационна дръжка.' })
  }
...
export function moveComposerHinge(state: VisualComposition, hingeId: string, positionRatio: number): VisualComposition { const target = state.components.find((item) => item.id === hingeId && item.type === 'HINGE'); if (!target) return { ...state, message: 'Изберете валидна демонстрационна панта.' }; const ratio = clampPositionRatio(positionRatio); return changed(state, { components: state.components.map((item) => item.id === hingeId ? { ...item, positionRatio: ratio, placement: `${Math.round(ratio * 100)}%` } : item), message: 'Концептуалната позиция на пантата е променена.' }) }
