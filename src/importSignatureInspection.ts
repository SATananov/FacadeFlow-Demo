...
export function detectImportFormat(bytes: ArrayBuffer, fileName: string): { format: DetectedImportFormat; version?: string; summary?: string } {
  ...
  if (!/<!\s*(?:DOCTYPE|ENTITY)\b/i.test(prefix) && /<Order(?:\s|>)/.test(prefix) && /<Generator>\s*SkyGlazing\s*<\/Generator>/.test(prefix)) return { format: 'SKYGLAZING_XML', summary: 'Разпозната SkyGlazing XML структура за отделна read-only проверка.' }
  if (fileName.toLowerCase().endsWith('.lte') && !prefix.includes('\0')) { const lines = prefix.split(/\r\n|\n|\r/).filter(Boolean); if (lines.length && lines.every((line) => line.length === 149 && /^[\x20-\x7e]+$/.test(line))) return { format: 'LTE', summary: 'Разпознати ASCII LTE записи с ширина 149 знака за отделна read-only проверка.' } }
  ...
  if (fileName.toLowerCase().endsWith('.drawing-import.simulation.json')) {
    try { const parsed: unknown = JSON.parse(new TextDecoder().decode(view)); if (isFacadeFlowSimulation(parsed)) return { format: 'FACADEFLOW_SIMULATION_JSON', version: parsed.schemaVersion, summary: `FacadeFlow simulation schema ${parsed.schemaVersion}; данните не са възстановени.` } } catch { return { format: 'UNKNOWN' } }
  }
