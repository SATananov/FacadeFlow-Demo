...
export function normalizeDwgDatabase(database: RawDatabase, options: DwgDecodeOptions = {}): DwgDecodeResult {
  ...
  if ((database.objects?.IMAGEDEF?.length ?? 0) > 0) warnings.push({ code: 'EXTERNAL_REFERENCE', message: 'Външни raster ресурси не се зареждат.', count: database.objects?.IMAGEDEF?.length })
  const sourceFonts = new Set((database.tables?.STYLE?.entries ?? []).map((entry) => String(entry.font ?? '').trim()).filter(Boolean))
  if (sourceFonts.size) warnings.push({ code: 'MISSING_FONT', message: `${sourceFonts.size} source font дефиниции използват локален fallback; външни font файлове не се зареждат.`, count: sourceFonts.size })
  ...
  if (longMTextWithoutWidth) warnings.push({ code: 'TEXT_LAYOUT_LIMITATION', message: `${longMTextWithoutWidth} дълги MTEXT записа нямат надеждна source width и остават без автоматично пренасяне.`, count: longMTextWithoutWidth })
