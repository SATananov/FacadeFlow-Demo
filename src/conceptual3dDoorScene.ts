...
export function buildDoorConceptualScene(state: DoorComposition, productWidth = 1, productHeight = 1) {
  ...
  for (const item of state.hardware) { const field = state.fields.find((value) => value.id === item.parentFieldId); if (!field) continue; const x0 = -.45 + field.rect.x * .9, y0 = .45 - (field.rect.y + field.rect.height) * .95, x = item.side === 'LEFT' ? x0 : x0 + field.rect.width * .9 - .018, y = y0 + (1 - item.positionRatio) * field.rect.height * .95; nodes.push(node(`door-${item.kind.toLowerCase()}-${item.id}`, item.id, item.kind === 'HANDLE' ? 'HANDLE_MARKER' : 'HINGE_MARKER', { x, y: y - .018, z: DEMO_DEPTHS.hardwareOffset, width: .018, height: .036, depth: .018 }, material('#26383c', '#26383c'), item.kind === 'HANDLE' ? 'Демонстрационна дръжка' : 'Демонстрационна панта', true)) }
