...
export function SelectedComponentContext({ component, onBackToProduct, onStandalone }: Props) {
  const template = getProductTemplate(component.sourceTemplateId)
  return <section className="component-context" aria-labelledby="component-context-title">
    <div><span className="context-badge">СИМУЛАЦИЯ</span><div><b id="component-context-title">Обработван детайл от изделие</b><p><strong>{component.id}</strong> · {component.role} · {templateCategoryLabels[template.libraryCategory]} / {template.name} · {component.nominalLength} mm · {component.orientation === 'horizontal' ? 'Хоризонтален детайл' : 'Вертикален детайл'}{component.directionConfirmed && component.openingDirection ? ` · ${component.openingDirection === 'right' ? 'Дясно отваряне' : 'Ляво отваряне'} (експертно потвърдено)` : ''}</p></div></div>
