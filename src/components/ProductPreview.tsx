...
export function ProductPreview({ product, project, profileCode, profileSystem, selectedComponentId, onSelectComponent, onOpenComponent, onClose }: Props) {
  ...
  return <div className="preview-overlay" role="presentation"><section ref={modal} className="preview-modal" role="dialog" aria-modal="true" aria-labelledby="preview-title" aria-describedby="preview-safety"><div className="preview-header"><div><span className="preview-badge">СИМУЛАЦИЯ</span><h2 id="preview-title">Визуализация на изделието</h2><p>Схема {template.displayNumber} — {template.name} · {profileSystem} · {project}</p></div><button ref={closeButton} type="button" className="preview-close" aria-label="Затвори визуализацията" onClick={onClose}>×</button></div>
    ...
    <div className="preview-footer"><p id="preview-safety"><b>Демонстрационна геометрия — изисква проверка от технолог.</b><br/>Посоката на символа е демонстрационна. Изгледът отвътре/отвън и страната на пантите предстоят за потвърждение от технолог.<br/>Примерните дължини и ъгли не са одобрени производствени изчисления.</p><div><button type="button" className="print-button" onClick={() => window.print()}>Разпечатай визуализация</button><button type="button" className="secondary-button" onClick={onClose}>Затвори</button></div></div>
  </section></div>
}
