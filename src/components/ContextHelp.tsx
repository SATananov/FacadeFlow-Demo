...
export function ContextHelp({ helpId }: { helpId: string }) {
  ...
  return <span className="context-help"><button ref={trigger} type="button" data-context-help-trigger="true" aria-label={`Помощ: ${entry.title}`} aria-expanded={open} aria-controls={popoverId} onClick={toggle}>?</button>{portal}</span>
}
