...
export function HelpCenter({ onClose, onStartTour }: Props) {
  ...
  return <div className="preview-overlay help-overlay ff-section-workspace" role="presentation"><section ref={dialog} className="help-center" role="dialog" aria-modal="true" aria-labelledby="help-title"><FacadeFlowWorkspaceHeader titleId="help-title" icon="help" eyebrow="Ръководство и проверка" title="Помощ за FacadeFlow" subtitle="Ръководството не променя текущите данни, геометрията или работния поток." onBack={onClose}/><div className="help-workspace-content"><div className="help-toolbar"><label>Търсене в помощта<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Напр. OCR, VERIFIED, X координата"/></label><button type="button" className="primary-button" onClick={onStartTour}>Започни обиколка</button></div><div className="help-layout"><nav aria-label="Раздели на помощта">{helpSections.map((section) => <button key={section.id} type="button" aria-current={active === section.id ? 'page' : undefined} onClick={() => navigate(section.id)}>{section.title}</button>)}</nav><div className="help-content" tabIndex={-1}>{visible.length ? visible.map((section) => <section key={section.id} id={`help-${section.id}`} onFocus={() => setActive(section.id)}><h3>{section.title}</h3>{section.paragraphs.map((text) => <p key={text}>{text}</p>)}{section.items && (section.id === 'quick-start' ? <ol>{section.items.map((item) => <li key={item}>{item}</li>)}</ol> : <ul>{section.items.map((item) => <li key={item}>{item}</li>)}</ul>)}</section>) : <p role="status">Няма намерени резултати. Опитайте с друга дума.</p>}</div></div></div></section></div>
}
