...
function ReviewColumn({ session, profiles, onReset }: { session: FacadeFlowAiSession; profiles: CatalogueProfile[]; onReset: () => void }) {
  ...
  return <aside className="ff-ai-review-column">
    ...
    <ol className="ff-ai-gate-list"><li className={hasScope ? 'done' : ''}><b>1</b><div><strong>Контекст на работата</strong><span>{hasScope ? FACADEFLOW_JOB_TYPE_LABELS[session.job.jobType!].title : session.job.demoScenario === 'KNOWLEDGE_BASE' ? 'Данни и каталози' : 'Не е избран'}</span></div></li><li className={hasInput || session.job.demoScenario === 'KNOWLEDGE_BASE' ? 'done' : ''}><b>2</b><div><strong>Източник / режим</strong><span>{hasInput ? FACADEFLOW_AI_INPUT_LABELS[session.job.inputMode!].title : session.job.demoScenario === 'KNOWLEDGE_BASE' ? 'База знания' : 'Не е избран'}</span></div></li><li className={demoPacketPrepared || hasStructuredProposal || (hasDescription && session.job.inputMode === 'DESCRIPTION') ? 'done' : ''}><b>3</b><div><strong>Обща структурирана спецификация</strong><span>{demoReviewPacket ? `ДЕМО пакет · ${demoReviewPacket.unresolved.length} неуточнени · AI ГЕНЕРИРАНО: НЕ` : hasStructuredProposal ? `Подготвено без AI автоматично извеждане · ${guidedProposal?.unresolved.length ?? 0} неуточнени` : 'Подготви общия пакет или продуктово предложение'}</span></div></li><li className={humanConfirmed || demoPacketReviewed ? 'done' : ''}><b>4</b><div><strong>Човешки преглед / потвърждение</strong><span>{humanConfirmed ? 'Изделието е потвърдено от човек' : demoPacketReviewed ? 'ДЕМО пакетът е прегледан от човек · изделието остава непотвърдено' : 'Задължително'}</span></div></li><li><b>5</b><div><strong>Проверка по правила</strong><span>Задължително — още не е изпълнена</span></div></li><li className={humanConfirmed ? 'ready' : ''}><b>6</b><div><strong>Преход към конструктора</strong><span>{humanConfirmed ? 'Готово за ръчен преход към конструктора · правилата остават задължителни' : demoPacketReviewed ? 'Остава заключено: прегледан пакет ≠ потвърдено изделие' : 'Заключено до човешко потвърждение'}</span></div></li></ol>
    ...
    <div className="ff-ai-job-summary"><span>Текуща сесия{session.job.demoScenario ? ' · ДЕМО' : ''}</span><b>{session.job.name || 'Без име'}</b><small>{session.job.reference || 'Без референция'} · {intakeStatusLabels[session.job.intakeStatus]}</small></div>
