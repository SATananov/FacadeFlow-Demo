...
interface Props { project: string; profile: Profile; orientation: Orientation; product: ProductParameters; productErrors: string[]; onProject: (v: string) => void; onProfile: (v: Profile) => void; onOrientation: (v: Orientation) => void; onProduct: (v: ProductParameters) => void; onProductPreview: () => void }
export function ProfilePanel({ project, profile, orientation, product, productErrors, onProject, onProfile, onOrientation, onProduct, onProductPreview }: Props) {
  ...
  return <aside className="panel profile-panel" aria-labelledby="profile-title">
    ...
    <ProductSection product={product} errors={productErrors} onChange={onProduct} onPreview={onProductPreview}/>
  </aside>
}
