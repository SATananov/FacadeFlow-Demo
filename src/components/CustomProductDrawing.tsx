...
export function CustomProductDrawing({ product, selectedFieldId, onSelectField, large = false, annotations = [], dimensionVisibility = defaultDimensionVisibility, gridVisible = false, gridStep = 50, onCursorCoordinates, snapPoint = null, snappingEnabled = false, zoom = 1 }: Props) {
  ...
  const updateCursorCoordinates = (event: PointerEvent<SVGSVGElement>) => {
    ...
    if (!matrix) return onCursorCoordinates(null)
    ...
    onCursorCoordinates({ x: Math.round(coordinates.x), y: Math.round(coordinates.y) })
  }

  return <svg className={`custom-product-drawing ${large ? 'large' : ''}`} viewBox={`0 0 ${viewWidth} ${viewHeight}`} role="img" aria-label={`Нестандартен прозорец ${product.width} на ${product.height} милиметъра`} onPointerMove={updateCursorCoordinates} onPointerLeave={() => onCursorCoordinates?.(null)}>
    ...
    {product.frameCreated && gridVisible && <CustomCoordinateGrid transform={transform} step={gridStep}/>}
