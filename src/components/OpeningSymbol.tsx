...
export function OpeningSymbol({ x, y, width, height, notation, selected = false }: Props) {
  const clipId = `opening-clip-${useId().replaceAll(':', '')}`
  const description = descriptions[notation]
  if (notation === 'FIXED' || notation === 'TILT_PLACEHOLDER' || notation === 'TILT_TURN_PLACEHOLDER') return <g role="img" aria-label={description}/>
