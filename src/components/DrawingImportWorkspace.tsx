...
import { ocrProgressStatusBg, ocrTargetFieldLabels } from '../importUiLabels'
...
export function DrawingImportWorkspace({ baseProduct, onPreview, onLoadVerified, onClose }: Props) {
  ...
  const [ocrMessage, setOcrMessage] = useState('Готово')
  ...
  const [combinedMessage, setCombinedMessage] = useState('Готово')
  ...
  const clearSource = () => {
    ...
    setSource(null); setSourceSession(null); setFoundationSession(null); setProducts([]); setPage(1); setDraft(blankDraft()); setEditingId(null); setErrors([]); setFileError(''); setOcrSelection(null); setOcrJobs([]); setOcrAudit([]); setActiveOcrJobId(null); setOcrMessage('Готово'); setCombinedSelection(null); setCombinedJob(null); setCombinedAuditEntries([]); setCombinedMessage('Готово'); setCombinedProcessing(false); setProvisionalDraft(null)
  ...
  const acceptFile = async (file?: File) => {
    ...
    try {
      ...
      releaseDrawingSource(source); setProducts([]); setPage(1); setDraft(blankDraft()); setEditingId(null); setErrors([]); setOcrSelection(null); setOcrJobs([]); setOcrAudit([]); setActiveOcrJobId(null); setOcrMessage('Готово'); setCombinedSelection(null); setCombinedJob(null); setCombinedAuditEntries([]); setCombinedMessage('Готово'); setCombinedProcessing(false); setProvisionalDraft(null)
  ...
  const startOcr = async () => {
    ...
    if (ocrSelection.rectangle.width * ocrSelection.rectangle.height > ocrLimits.maximumCropPixelArea) { setOcrMessage(`ГРЕШКА: зоната надвишава ${ocrLimits.maximumCropPixelArea.toLocaleString('bg-BG')} пиксела.`); return }
    ...
    setOcrJobs((items) => [initial, ...items].slice(0, ocrLimits.maximumRetainedJobs)); setActiveOcrJobId(id); setOcrMessage('ОБРАБОТКА: локалното OCR работи…')
    try {
      const result = await runLocalOcr(ocrSelection.imageDataUrl, (progress, message) => { setOcrMessage(`ОБРАБОТКА: ${ocrProgressStatusBg(message)} ${Math.round(progress * 100)}%`); setOcrJobs((items) => items.map((item) => item.id === id ? { ...item, progress } : item)) }, (control) => { ocrControl.current = control })
      ...
      setOcrMessage(result.normalizedText ? 'ЗАВЪРШЕНО: резултатът е само предложение за човешка проверка.' : 'ЗАВЪРШЕНО: не е разпознат текст в избраната зона.')
    } catch (reason) {
      const cancelled = cancelledOcrJobs.current.has(id)
      setOcrJobs((items) => items.map((item) => item.id === id ? { ...item, state: cancelled ? 'CANCELLED' : 'FAILED', error: reason instanceof Error ? reason.message : 'Грешка в локалното OCR разпознаване', completedAt: new Date().toISOString() } : item))
      setOcrMessage(cancelled ? 'ПРЕКРАТЕНО: разпознаването е прекратено.' : 'ГРЕШКА: локалното OCR не завърши успешно.')
    } finally { ocrControl.current = null; currentOcrJobId.current = null; cancelledOcrJobs.current.delete(id) }
  }
  const cancelOcr = async () => { const control = ocrControl.current; if (currentOcrJobId.current) cancelledOcrJobs.current.add(currentOcrJobId.current); ocrControl.current = null; await control?.cancel(); setOcrMessage('ПРЕКРАТЕНО: разпознаването е прекратено.') }
  ...
  const applyCandidate = (jobId: string, candidate: OcrCandidate, field: OcrTargetField, proposed: string) => {
    ...
    if (!window.confirm(`Да се приложи ли OCR предложението?\nПоле: ${ocrTargetFieldLabels[field]}\nСтара стойност: ${previous}\nПредложена стойност: ${value}\n\nПотвърдете само след сравнение с оригиналния чертеж.`)) return
    if (field === 'width' || field === 'height') { const numeric = Number(value.replace(',', '.')); if (!Number.isFinite(numeric) || numeric < ocrLimits.minimumDimension || numeric > ocrLimits.maximumDimension) { setOcrMessage('ГРЕШКА: предложената размерна стойност е извън допустимите демонстрационни граници.'); return }; changeDraft({ [field]: numeric }) }
  ...
  const analyzeCombined = async () => {
    ...
    if (combinedSelection.rectangle.width * combinedSelection.rectangle.height > ocrLimits.maximumCropPixelArea) { setCombinedMessage(`ГРЕШКА: зоната надвишава ${ocrLimits.maximumCropPixelArea.toLocaleString('bg-BG')} пиксела.`); return }
    setCombinedProcessing(true); setCombinedMessage('ОБРАБОТКА: локален анализ на геометрията и означенията…')
    try { const job = await runCombinedAnalysis(combinedSelection, source.metadata.sha256, setCombinedMessage, (control) => { ocrControl.current = control }); setCombinedJob(job); setCombinedAuditEntries([]); setProvisionalDraft(generateProvisionalDraft(job, source.metadata.fileName)); setCombinedMessage('ЗАВЪРШЕНО: създадена е автоматична чернова за човешка проверка.') }
    catch (reason) { setCombinedMessage(`ГРЕШКА: ${reason instanceof Error ? reason.message : 'Комбинираният анализ не завърши.'}`) }
  ...
  return <div className="preview-overlay drawing-import-overlay ff-section-workspace" role="presentation"><section ref={modal} className="drawing-import-modal" data-help-id="import-workspace" role="dialog" aria-modal="true" aria-labelledby="drawing-import-title" aria-describedby="drawing-import-safety">
    ...
    <div id="drawing-import-safety" className="ff-workspace-safety"><b>Симулация · локална обработка.</b> Файлът остава в паметта на браузъра. Няма сървърно качване, автоматично производствено тълкуване или машинен изход.</div>
    ...
    {!selectedRoute ? <ImportFormatChooser onSelect={setSelectedRoute}/> : selectedRoute === 'SKYGLAZING' ? <SkyGlazingImportWorkspace onBack={backToFormatChoice}/> : selectedRoute === 'CAD' ? <DwgViewerWorkspace onBack={backToFormatChoice}/> : foundationSession ? <FoundationSourceInspection session={foundationSession} onClear={clearSource} onBack={backToFormatChoice}/> : !source ? <section className="drawing-file-start">
      ...
      <section className="captured-products" aria-labelledby="captured-products-title"><div className="captured-products-heading"><div><h3 id="captured-products-title">Заснети изделия</h3><p>{products.length} ръчно въведени записа</p></div><div><label>Страница<select value={pageFilter} onChange={(event) => setPageFilter(event.target.value)}><option value="ALL">Всички</option>{Array.from({ length: source.metadata.pageCount }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label><label>Статус<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)}><option value="ALL">Всички</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div></div>
        {visibleProducts.length ? <div className="captured-product-list">{visibleProducts.map((item) => <article key={item.id}><div><b>{item.productReference}</b><small>{item.id}</small><span>{item.templateId} · {item.width} × {item.height} mm · {item.quantity} бр. · стр. {item.sourcePage}</span><span className={`review-status ${item.status.toLowerCase()}`}>{statusLabels[item.status]}</span></div><div><button type="button" onClick={() => onPreview(toProduct(item, baseProduct), item.projectReference)}>Преглед</button><button type="button" onClick={() => edit(item)}>Редактирай</button><button type="button" onClick={() => duplicate(item)}>Дублирай</button><button type="button" disabled={item.status !== 'VERIFIED'} title={item.status !== 'VERIFIED' ? 'Само проверен запис може да бъде зареден.' : undefined} onClick={() => onLoadVerified(item)}>Зареди в работния поток</button><button type="button" className="danger-button" onClick={() => { if (window.confirm(`Да се изтрие ли ${item.productReference}?`)) setProducts((items) => items.filter(({ id }) => id !== item.id)) }}>Изтрий</button></div></article>)}</div> : <p className="empty-captured">Няма записи за избраните филтри.</p>}
      </section>
      <footer className="drawing-import-footer"><p><b>Само симулация.</b> Данните изискват човешка проверка и не са готови за машина.</p><button type="button" className="export" disabled={!products.length && !provisionalDraft} onClick={() => exportDrawingImportSimulation({ source: source.metadata, products, validation: { valid: products.length > 0 || Boolean(provisionalDraft), errors: products.length || provisionalDraft ? [] : ['Няма заснети изделия или автоматична чернова.'] }, ocrJobs, ocrAudit, combinedAnalysis: combinedJob ?? undefined, combinedAudit: combinedAuditEntries, provisionalDraft: provisionalDraft ?? undefined })}>Експортирай симулационен JSON</button></footer>
    </>}
    </div>
  </section></div>
}
