...
export function ProjectStructureBuilder({ session, setSession }: Props) {
  ...
  return <section className="ff-project-structure" aria-labelledby="ff-project-structure-title">
    <div className="ff-project-structure-head">
      <div><span>СТРУКТУРА · ПО ЖЕЛАНИЕ</span><h4 id="ff-project-structure-title">Къде принадлежи изделието?</h4><p>Добавяй само нивата, които реално съществуват. Етаж, помещение, фасада и позиция не са задължителни. Единично изделие може да остане без йерархия.</p></div>
      <div className="ff-project-active-path"><span>АКТИВЕН ПЪТ</span><b>{activePath.length ? activePath.map((node) => node.label).join(' → ') : 'Без структура / директно изделие'}</b><small>Този път се записва в структурираното изделие при човешка проверка.</small></div>
    </div>
    ...
    {structure.nodes.length === 0 ? <div className="ff-project-structure-empty"><b>Няма добавена йерархия.</b><span>Това е валидно за единично изделие, малка поръчка или когато проектната структура още не е известна.</span></div> : <div className="ff-project-structure-list">{structure.nodes.map((node) => { const active = structure.activeNodeId === node.id; const depth = projectStructureNodeDepth(structure, node.id); return <article key={node.id} className={active ? 'active' : ''} style={{ '--ff-project-depth': depth } as CSSProperties}><div><span>{PROJECT_NODE_LABELS[node.kind]}</span><b>{node.label}</b><small>{node.source === 'MANUAL' ? 'Ръчно' : 'Извлечено'} · ЧЕРНОВА · САМО В ТЕКУЩАТА СЕСИЯ</small></div><div><button type="button" className={active ? 'selected' : ''} onClick={() => setSession((current) => selectFacadeFlowProjectNode(current, node.id))}>{active ? '✓ Използва се за изделието' : 'Използвай за изделието'}</button><button type="button" className="danger" onClick={() => setSession((current) => removeFacadeFlowProjectNode(current, node.id))}>Премахни</button></div></article> })}</div>}

    <div className="ff-project-structure-foot"><button type="button" className={!structure.activeNodeId ? 'selected' : ''} onClick={() => setSession((current) => selectFacadeFlowProjectNode(current, null))}>Без структура / директно изделие</button><span data-safety="PROJECT MODEL: SESSION ONLY · PERSISTENCE: NO · MACHINE READY: NO">МОДЕЛ НА ПРОЕКТА: САМО В ТЕКУЩАТА СЕСИЯ · ПОСТОЯННО СЪХРАНЕНИЕ: НЕ · ГОТОВО ЗА МАШИНА: НЕ</span></div>
  </section>
}
