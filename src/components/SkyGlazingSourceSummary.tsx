...
import { detectedImportFormatLabels, importSupportStatusLabels } from '../importUiLabels'
...
export function SkyGlazingSourceSummary({ source, onClear }: Props) {
  return <article className="sky-source-card">
    <header><span className="route-badge">{detectedImportFormatLabels[source.detectedFormat]}</span><b>{source.fileName}</b></header>
    <dl>
      <div><dt>Разширение</dt><dd>.{source.extension}</dd></div>
      <div><dt>Открит формат</dt><dd>{detectedImportFormatLabels[source.detectedFormat]}</dd></div>
      <div><dt>Размер</dt><dd>{(source.sizeBytes / 1024).toFixed(2)} KB</dd></div>
      <div><dt>Статус</dt><dd>{importSupportStatusLabels[source.supportStatus]}</dd></div>
    ...
    <p className="sky-simulation-warning">СИМУЛАЦИЯ · локално доказателство само за преглед · без машинен изход</p>
