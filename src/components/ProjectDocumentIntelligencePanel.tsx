...
export function ProjectDocumentIntelligencePanel({ profiles, setSession, onOpenImportCenter }: Props) {
  ...
  return <section className="ff-ai-document-intelligence" aria-labelledby="ff-ai02-title">
    ...
    {selectedGroup && <section className="ff-ai02-transfer"><div><span>03 · SAFE HANDOFF</span><h4>{selectedGroup.mark || 'Избрана позиция'} → структурирания Human Review формуляр</h4><p>Прехвърлят се само съвместими стойности. Конфликтните полета остават неуточнени. Многополевата топология остава Product Intent evidence и не се превръща автоматично в CAD.</p></div><button type="button" className="primary-button" onClick={transferSelected}>Прехвърли безопасните стойности</button></section>}
