...
export function PromptInterpretationPanel({ session, profiles, setSession, onOpenAi04Constructor, onAnalysisComplete }: {
  ...
  return <section className="ff-ai-prompt-interpreter" aria-live="polite">
    <div className="ff-ai-prompt-action">
      <button type="button" disabled={!sourceText} onClick={analyse} title={stale ? 'Разчети отново и обнови чертежа' : 'Разчети описанието и начертай'}><FacadeFlowIcon name="ai"/> {stale ? 'Разчети отново' : 'Разчети описанието'} {stale ? 'и обнови чертежа' : 'и начертай'}</button>
