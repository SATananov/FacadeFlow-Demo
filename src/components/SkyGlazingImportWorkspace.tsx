...
export function SkyGlazingImportWorkspace({ onBack }: { onBack: () => void }) {
  ...
  return <section className="sky-workspace" aria-labelledby="sky-workspace-title">
    <header><div><span className="route-badge">САМО ЗА ПРЕГЛЕД · ЛОКАЛНО</span><h3 id="sky-workspace-title">SkyGlazing XML / LTE сравнение</h3><p>Източниците остават само в паметта на браузъра. Няма качване, запис, машинно тълкуване или експортиране.</p></div><button type="button" onClick={onBack}>Назад към форматите</button></header>
    ...
    {busy && <p aria-live="polite">Локална проверка, SHA-256 и безопасно извличане само за преглед…</p>}
    ...
    {xml && <section className="sky-section" aria-labelledby="xml-summary-title"><h3 id="xml-summary-title">XML обобщение</h3><div className="sky-counts"><span>Генератор: {xml.inspection.generator || '—'}</span><span>Версия: {xml.inspection.version || '—'}</span><span>Единица: {xml.inspection.unit || '—'}</span><span>Проект: {xml.inspection.projectName || '—'}</span><span>Bar записи: {xml.inspection.barCount}</span><span>Piece записи: {xml.inspection.pieceCount}</span><span>Work записи: {xml.inspection.workCount}</span><span>Уникални баркодове: {xml.inspection.uniqueBarcodeCount}</span><span>Уникални DXF профили: {xml.inspection.uniqueDxfProfileCount}</span></div><div className="sky-table-wrap"><table><thead><tr><th>№</th><th>DXF профил</th><th>MaxY</th><th>MaxZ</th><th>Баркод</th><th>Дължина</th><th>sxB</th><th>dxB</th><th>sxC</th><th>dxC</th><th>Операции</th><th>Наблюдавани имена</th></tr></thead><tbody>{xml.inspection.pieces.map((piece) => <tr key={`${piece.normalizedBarcode}-${piece.originalRecordIndex}`}><td>{piece.originalRecordIndex + 1}</td><td>{piece.dxfProfileName}</td><td>{piece.maxY}</td><td>{piece.maxZ}</td><td>{piece.barcode}</td><td>{piece.length}</td><td>{piece.sxB}</td><td>{piece.dxB}</td><td>{piece.sxC}</td><td>{piece.dxC}</td><td>{piece.operationCount}</td><td>{piece.operationNames.join(', ') || '—'}</td></tr>)}</tbody></table></div><p className="sky-boundary-note">Параметрите са показани като наблюдавана структура, без потвърдена машинна семантика.</p><details><summary>Сурова XML структура — безопасно показан текст</summary><pre>{xml.inspection.rawStructuralText}</pre></details></section>}
    {lte && <section className="sky-section" aria-labelledby="lte-summary-title"><h3 id="lte-summary-title">LTE обобщение</h3><div className="sky-counts"><span>Записи: {lte.inspection.recordCount}</span><span>Фиксирана ширина: {lte.inspection.fixedRecordWidth ?? 'нееднаква'}</span><span>Уникални баркодове: {lte.inspection.uniqueBarcodeCount}</span><span>Профилни групи: {lte.inspection.profileGroupCount}</span></div><p className="sky-boundary-note">Всички други диапазони с фиксирана ширина остават неразрешени до потвърждение от домейн експерт.</p><div className="sky-table-wrap"><table><thead><tr><th>Ред</th><th>Профилна стойност</th><th>Сурова стойност за дължина</th><th>Баркод</th><th>Оригинален неизменен ред</th></tr></thead><tbody>{lte.inspection.records.map((record) => <tr key={`${record.lineNumber}-${record.normalizedBarcode}`}><td>{record.lineNumber}</td><td>{record.profileToken}</td><td><code>{record.rawLengthToken}</code></td><td>{record.barcode}</td><td><code className="sky-raw-line">{record.originalLine}</code></td></tr>)}</tbody></table></div></section>}
    {comparison ? <SkyGlazingComparisonTable comparison={comparison} /> : <p className="sky-pair-prompt">Изберете и двата файла за сравнение само по баркод.</p>}
    <footer className="sky-readonly-footer"><b>Само проверка за преглед.</b> Не се създават, редактират или експортират XML/LTE/DWG файлове и Work записите не създават операции.</footer>
  </section>
}
