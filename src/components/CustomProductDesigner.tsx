...
export function CustomProductDesigner({ initial, profiles, activeProfiles, selectedComponentId, onCommit, onOpenComponent, onClose }: Props) {
  ...
  return <div className="preview-overlay custom-designer-overlay"><section className="custom-designer" role="dialog" aria-modal="true" aria-labelledby="custom-designer-title">
    ...
    <nav className="structured-workflow" aria-label="Професионална последователност">
    ...
    <div className="view-switch" role="group" aria-label="Изглед на нестандартното изделие"><button type="button" aria-pressed={view === '2D'} onClick={() => setView('2D')}>2D чертеж</button><button type="button" aria-pressed={view === '3D'} disabled={product.status !== 'VERIFIED'} title={product.status !== 'VERIFIED' ? '3D прегледът е достъпен след човешка проверка.' : undefined} onClick={() => setView('3D')}>3D преглед</button><button type="button" aria-pressed={view === 'SPLIT'} disabled={product.status !== 'VERIFIED'} title={product.status !== 'VERIFIED' ? 'Разделеният 3D преглед е достъпен след човешка проверка.' : undefined} onClick={() => setView('SPLIT')}>Разделен изглед</button></div>
