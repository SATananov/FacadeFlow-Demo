...
export function CustomProductDesigner({ initial, profiles, activeProfiles, selectedComponentId, onCommit, onOpenComponent, onClose }: Props) {
  ...
  const apply = (next: CustomProduct) => { const status = validateCustomProduct(next, profiles).valid ? 'NEEDS_REVIEW' as const : 'DRAFT' as const; const normalized = { ...next, status, humanReviewConfirmed: false, updatedAt: new Date().toISOString() }; if (!onCommit(product, normalized)) return; setHistory((value) => pushHistory(value, normalized)); setReviewChecked(false) }
  const replaceGeometry = (geometry: CustomGeometryNode) => apply({ ...product, geometry })
  const changeSplit = (orientation: SplitOrientation, position: number) => selectedNode?.kind === 'SPLIT' ? replaceGeometry(updateGeometryNode(product.geometry, selectedNode.id, (node) => node.kind === 'SPLIT' ? { ...node, orientation, position } : node)) : replaceGeometry(splitField(product.geometry, selectedFieldId, orientation, position))
  ...
  return <div className="preview-overlay custom-designer-overlay"><section className="custom-designer" role="dialog" aria-modal="true" aria-labelledby="custom-designer-title">
    ...
    {view !== '3D' && <div className="custom-designer-grid"><div className="custom-canvas"><div className="custom-canvas-controls"><button onClick={() => setZoom((value) => Math.min(1.8, value + .15))}>Увеличи</button><button onClick={() => setZoom((value) => Math.max(.55, value - .15))}>Намали</button><button onClick={() => setZoom(1)}>Побери / нулирай</button><button onClick={() => setLargePreview((value) => !value)}>{largePreview ? 'Редактор' : 'Преглед на изделието'}</button></div><div className="custom-drawing-scroll"><div style={{ width: `${zoom * 100}%` }}><CustomProductDrawing product={product} selectedFieldId={selectedFieldId} onSelectField={setSelectedFieldId} large={largePreview}/></div></div><div className="current-assignments">Активни: каса {activeProfiles.FRAME ?? '—'} · крило {activeProfiles.SASH ?? '—'} · делител {activeProfiles.MULLION ?? '—'}</div></div><SelectedFieldPanel node={selectedNode} fieldWidth={selectedProjection?.rect.width ?? 0} fieldHeight={selectedProjection?.rect.height ?? 0} profiles={profiles} errors={validation.fieldErrors[selectedFieldId] ?? []} onSplit={changeSplit} onLeaf={(patch: Partial<CustomLeafNode>) => replaceGeometry(updateGeometryNode(product.geometry, selectedFieldId, (node) => node.kind === 'LEAF' ? { ...node, ...patch } : node))} onRemoveSplit={() => { if (!selectedNode || selectedNode.kind !== 'SPLIT') return; if (geometryHasNestedSplit(selectedNode) && !window.confirm('Разделянето съдържа вложена геометрия. Да бъде ли премахната?')) return; const parentId = selectedNode.id.replace(/^split-/, ''); replaceGeometry(removeSplit(product.geometry, selectedNode.id)); setSelectedFieldId(parentId) }}/></div>}
    {validation.errors.length > 0 && <div className="custom-validation inline-errors" role="alert"><b>Черновата изисква корекции:</b><ul>{validation.errors.map((error) => <li key={error}>{error}</li>)}</ul></div>}
    <CustomProductSummary components={components} selectedId={selectedSummaryId} onSelect={setSelectedSummaryId} onOpen={(component) => { setSelectedSummaryId(component.id); onOpenComponent(component) }}/>
    <footer className="custom-designer-footer"><div><p className="custom-production-warning">Номиналната дължина не е производствен размер. Формулите за сглобка и отнемане предстоят за потвърждение.</p><label><input type="checkbox" checked={reviewChecked} onChange={(event) => setReviewChecked(event.target.checked)}/> Проверих размерите, профилите, разделянето и отваряемостта.</label><small>VERIFIED означава проверена от човек симулация, не технологично или машинно одобрение.</small></div><div><button onClick={() => exportCustomProduct(product, profiles, components, validation)}>Експортирай custom simulation JSON</button><button className="primary" disabled={!validation.valid || !reviewChecked} onClick={verify}>Потвърди след човешка проверка</button></div></footer>
  </section></div>
}
