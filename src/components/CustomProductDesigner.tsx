...
export function CustomProductDesigner({ initial, profiles, activeProfiles, selectedComponentId, onCommit, onOpenComponent, onClose }: Props) {
  ...
  const product = history.present, ai04Handoff = product.ai04Handoff, validation = useMemo(() => validateCustomProduct(product, profiles), [product, profiles]), components = useMemo(() => generateCustomComponents(product, profiles), [product, profiles])
  const activeProfileOverlap = useMemo(() => resolveActiveProfileSystemOverlap(profiles, activeProfiles), [profiles, activeProfiles])
  const effectiveSashOverlapMm = activeProfileOverlap.state === 'APPLIES' ? activeProfileOverlap.sashOverlapMm : null
  const profileOverlapAudit = activeProfileOverlap.state === 'APPLIES'
    ? <div className="custom-profile-overlap-audit" data-overlap-state={activeProfileOverlap.state} data-profile-system={activeProfileOverlap.systemLabel} data-sash-overlap-mm={activeProfileOverlap.sashOverlapMm}><span>Система <b>{activeProfileOverlap.systemLabel}</b></span><span>Застъпване на крилото <strong>{activeProfileOverlap.sashOverlapMm} mm</strong></span><small>работна човешки прегледана стойност · Не е производствено потвърждение</small></div>
    : null
  ...
  const header = <FacadeFlowWorkspaceHeader titleId="custom-designer-title" icon="technical-detail" eyebrow={`Нестандартно изделие · ${product.status}`} title="Конструктор на нестандартен прозорец" subtitle="Структуриран параметричен и CAD работен поток · без производствени формули." onBack={onClose}/>
  ...
  const viewport = <>{view !== '2D' && (
        ...
        <div className="custom-drawing-zoom" style={{ width: `${zoom * 100}%`, height: `${zoom * 100}%` }}>
          <div className="custom-drawing-stage">
            ...
            <CustomProductDrawing
             ...
             sashOverlapMm={effectiveSashOverlapMm}/>
      ...
      <div className="current-assignments">Активни: каса {activeProfiles.FRAME ?? '—'} · крило {activeProfiles.SASH ?? '—'} · делител {activeProfiles.MULLION ?? '—'}</div>{profileOverlapAudit}
    </div>}</>
  ...
  const status = <>{ai04Handoff && <section className="custom-ai04-handoff-banner" aria-label="Проследимост на предложението"><div><span>РЕДАКТИРУЕМА ГЕОМЕТРИЯ ОТ ПРЕГЛЕДАНО ПРЕДЛОЖЕНИЕ</span><strong>{ai04Handoff.mark || 'Без марка'} · {ai04Handoff.geometryBasis === 'EQUAL_DISTRIBUTION_PROPOSAL' ? 'прието концептуално равномерно разпределение' : ai04Handoff.geometryBasis === 'EXPLICIT_DIVIDERS' ? 'делители от доказателствата' : 'едно поле'}</strong><p>Източник: {ai04Handoff.sourceKind === 'PROMPT' ? 'Описание' : ai04Handoff.sourceKind === 'DOCUMENT' ? 'Документ' : ai04Handoff.sourceKind === 'SKETCH' ? 'Скица' : 'Ръчно'} · Доказателства: {ai04Handoff.evidenceCount}. Това е редактируема симулационна чернова; профили, отваряемост и всички последващи промени остават за проверка в конструктора.</p></div><dl><div><dt>Предложение, одобрено от човек</dt><dd>ДА</dd></div><div><dt>Автоматичен преход</dt><dd>НЕ</dd></div><div><dt>Правила валидирани</dt><dd>НЕ</dd></div><div><dt>Готово за машина</dt><dd>НЕ</dd></div></dl></section>}{validation.errors.length > 0 && <div className="custom-validation inline-errors" role="alert"><b>Черновата изисква корекции:</b><ul>{validation.errors.map((error) => <li key={error}>{error}</li>)}</ul></div>}<section className="custom-component-list-region" aria-labelledby="custom-component-list-title"><div className="custom-component-list-heading"><strong id="custom-component-list-title">Компоненти <span>({components.length})</span></strong><button type="button" aria-expanded={componentsVisible} aria-controls="custom-component-list-content" onClick={() => setComponentsVisible((visible) => !visible)}>{componentsVisible ? 'Скрий компонентите' : 'Покажи компонентите'}</button></div><div id="custom-component-list-content" className="custom-component-list-content" hidden={!componentsVisible}><CustomProductSummary components={components} selectedId={selectedSummaryId} onSelect={setSelectedSummaryId} onOpen={(component) => { setSelectedSummaryId(component.id); onOpenComponent(component) }}/></div></section><footer className="custom-designer-footer"><div><p className="custom-production-warning">Номиналната дължина не е производствен размер. Формулите за сглобка и отнемане предстоят за потвърждение.</p><label><input type="checkbox" checked={reviewChecked} onChange={(event) => setReviewChecked(event.target.checked)}/> Проверих размерите, профилите, разделянето и отваряемостта.</label><small>„Проверено“ означава симулация, проверена от човек, а не технологично или машинно одобрение.</small></div><div><button onClick={() => exportCustomProduct(product, profiles, components, validation)}>Експортирай JSON на симулационната чернова</button><button className="primary" disabled={!validation.valid || !reviewChecked} onClick={verify}>Потвърди след човешка проверка</button></div></footer></>
  return <div className="preview-overlay custom-designer-overlay ff-section-workspace"><DrawingWorkspaceShell labelId="custom-designer-title" className="custom-designer" header={header} progress={progress} settings={settings} toolbar={toolbar} viewport={viewport} properties={properties} status={status}/></div>
}
