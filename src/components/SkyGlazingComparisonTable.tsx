...
import { skyComparisonStatusLabels } from '../importUiLabels'
...
export function SkyGlazingComparisonTable({ comparison }: { comparison: SkyGlazingComparison }) {
  ...
  return <section className="sky-section" aria-labelledby="sky-comparison-title">
    ...
    <p className="sky-boundary-note">Авторитетно е единствено точното съвпадение на баркода след премахване на външните интервали. Дължини, ъгли и останали диапазони не се тълкуват и не създават автоматичен конфликт.</p>
    <div className="sky-counts"><span>XML записи: {comparison.xmlRecordCount}</span><span>LTE записи: {comparison.lteRecordCount}</span>{Object.entries(comparison.counts).map(([key, count]) => <span key={key}>{skyComparisonStatusLabels[key as SkyGlazingComparisonStatus]}: {count}</span>)}</div>
    ...
    <div className="sky-table-wrap"><table><thead><tr><th>Статус</th><th>Баркод</th><th>XML профил</th><th>LTE профилна стойност</th><th>Обяснение</th></tr></thead><tbody>{records.map((item, index) => <tr key={`${item.status}-${item.normalizedBarcode}-${index}`}><td><span className={`sky-status ${item.status.toLowerCase()}`}>{skyComparisonStatusLabels[item.status]}</span></td><td>{item.normalizedBarcode || '—'}</td><td>{item.xmlRecord?.dxfProfileName ?? '—'}</td><td>{item.lteRecord?.profileToken ?? '—'}</td><td>{item.explanation}</td></tr>)}</tbody></table></div>
