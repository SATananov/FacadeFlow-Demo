...
export function DwgVisualTextEditorPanel({ state, onMode, onState, onHide, onResetSource, onUndo, onClear, onUpdateNote, onRemoveNote }: Props) {
  ...
  return <section className="dwg-text-editor" aria-labelledby="dwg-text-editor-title">
    <header><div><h4 id="dwg-text-editor-title">Визуален текстов редактор</h4><p>Изходният текст остава непроменен. Редакциите са само за текущата сесия на браузъра.</p></div><span>САМО ВИЗУАЛНО</span></header>
    <div className="dwg-text-editor-toolbar">
      ...
      <button type="button" disabled={!selected} aria-pressed={state.mode === 'MOVE_SOURCE'} onClick={() => onMode('MOVE_SOURCE')}>Премести с плъзгане</button>
      ...
      <button type="button" disabled={!selected} onClick={onResetSource}>Възстанови изходния текст</button>
      <button type="button" disabled={!state.history.length} onClick={onUndo}>Отмени</button>
      <button type="button" disabled={!state.sourceEdits.length && !state.notes.length} onClick={onClear}>Изчисти всичко</button>
    </div>
    <div className="dwg-text-editor-status" role="status">{state.mode === 'SELECT_SOURCE' ? 'Щракнете върху изходен текст.' : state.mode === 'MOVE_SOURCE' ? 'Хванете избрания текст и го преместете с плъзгане.' : state.mode === 'DRAW_BOX' ? 'Изтеглете правоъгълник около желаното поле.' : state.mode === 'PLACE_NOTE' ? 'Щракнете върху чертежа, за да поставите новия текст.' : selected ? `Избран изходен текст ${state.selectedEntityIndex! + 1}.` : 'Изберете действие.'}</div>
