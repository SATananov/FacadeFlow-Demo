...
test('06C.2 catalogue UI shows real evidence separately and requires an explicit role review before catalogue save', () => {
  const ui = read('src/components/ProfileCatalogue.tsx')
  ...
  assert.match(ui, /РЕАЛНИ SOURCE EVIDENCE/)
  assert.match(ui, /РОЛЯ: .*НЕПОТВЪРДЕНА/)
  assert.match(ui, /reviewEvidenceAs\(item\.id, role\)/)
  assert.match(ui, /Прегледай като:/)
  assert.match(editor, /РЕАЛЕН SOURCE EVIDENCE/)
  assert.match(editor, /Провери ролята, системата и името преди запис/)
  assert.match(css, /catalogue-evidence-grid/)
...
test('06C.2 adds only local read-only evidence/audit logic and keeps production safety locked', () => {
  const files = [read('src/nadezhdaCatalogueEvidence.ts'), read('src/components/ProfileCatalogue.tsx'), read('src/aiGuidedProduct.ts'), read('scripts/audit-nadezhda-evidence.mjs')].join('\n')
