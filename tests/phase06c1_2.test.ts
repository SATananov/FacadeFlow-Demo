...
test('06C.1.2 UI exposes evidence handoff and keeps custom CAD as an explicit no-handoff path', () => {
  ...
  assert.match(ai, /Конструкторът чака човешко потвърждение/)
  assert.match(ai, /Отвори CAD без прехвърляне на AI данни/)
  assert.match(constructor, /AI → ПРЕХОД КЪМ КОНСТРУКТОРА/)
